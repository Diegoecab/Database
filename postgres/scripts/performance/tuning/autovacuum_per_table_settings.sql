-- ------------------------------------------------------------------------------
-- File       : autovacuum_per_table_settings.sql
-- Purpose    : postgres performance/tuning helper: autovacuum per table settings.
-- Engine     : postgres
-- Category   : performance/tuning
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : Run with the target database client: autovacuum_per_table_settings.sql
-- Parameters : MI, SS, float8
-- Risk       : CHANGES
...
--ALTER TABLE test SET (autovacuum_vacuum_scale_factor = 0, autovacuum_vacuum_threshold = 100);
--select relname, reloptions from pg_class where relname='test';
--ALTER TABLE test RESET (autovacuum_vacuum_scale_factor,autovacuum_vacuum_threshold);
