-- ------------------------------------------------------------------------------
-- File       : get_ddl_tablespaces.sql
-- Purpose    : Oracle storage, ASM, ACFS or tablespace helper: get ddl tablespaces (1).
-- Category   : storage
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : @get_ddl_tablespaces (1).sql
...
WHERE tablespace_name != 'SYSTEM' and tablespace_name !='TEMP';
...
dbms_output.put_line ('prompt Creando tablespace '||ts_rec.tablespace_name);
...
||' SIZE '||df_rec.bytes/1024/1024||'M REUSE ');
...
dbms_output.put_line (' MAXSIZE '||df_rec.maxbytes/1024/1024||'M');
end if;
end if;
END LOOP;
