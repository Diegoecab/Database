...
select lock_user,machine,SQL_ID,blocked_user,blocked_machine,bloqued_SQL_ID, min(minutes_in_wait), max(minutes_in_wait),count(*) from (
SELECT
s1.username lock_user,
s1.machine,
s1.sid,
...
AND l2.id2 = l2.id2
order by s2.seconds_in_wait
) group by lock_user,machine,SQL_ID,blocked_user,blocked_machine,bloqued_SQL_ID
order by SQL_ID
/
