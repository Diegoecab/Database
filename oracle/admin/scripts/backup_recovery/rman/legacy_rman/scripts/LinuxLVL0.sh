...
# ------------------------------------------------------------------------------
# File       : LinuxLVL0.sh
# Purpose    : Oracle RMAN backup, restore or recovery helper: LinuxLVL0.
...
rman catalog rman/oracle@rman target / log=$RMAN_LOG/rmanVTRPROD_inc_0_$FECHA.log<<EOF
run {
     sql 'alter system archive log current';
     change archivelog all validate;
     backup as compressed backupset incremental level=0 database include current controlfile format  '/u01/rman/VTRPROD/backup/bk0_%d_%s_%t_%U.bak' tag=Semanal_Level0 archivelog all format '/u01/rman/VTRPROD/backup/arc_%d_%s_%T.bak';
...
date >> $RMAN_LOG/rmanVTRPROD_inc_0_$FECHA.log
