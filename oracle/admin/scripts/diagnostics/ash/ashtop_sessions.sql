-- ------------------------------------------------------------------------------
-- File       : ashtop_sessions.sql
-- Purpose    : Oracle diagnostic query/report helper: ashtop sessions.
-- Category   : diagnostics/ash
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : @ashtop_sessions.sql
...
-- ------------------------------------------------------------------------------
--
--@ash/ashtop_sessions sysdate-1 sysdate username<>'SYS'
--@ash/ashtop_sessions to_date('06-05-20:10:00','DD-MM-YY:HH24:MI') sysdate username<>'SYS'
...
set verify off
 ...
 select
  ...
  from (
          select
        session_id||':'||session_serial# as "SID:SERIAL",
		username,
        count(*) as db_time,
     ...
     group by
        session_id||':'||session_serial#, username
     order by
        count(*) desc)
  where
     rownum <=5;