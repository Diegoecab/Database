-- ------------------------------------------------------------------------------
-- File       : ashtop_by_username.sql
-- Purpose    : Oracle diagnostic query/report helper: ashtop by username.
-- Category   : diagnostics/ash
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : @ashtop_by_username.sql
...
--
--------------------------------------------------------------------------------
--ashtop_by_username.sql
--@ash/ashtop_by_username.sql <fromtime> <totime>
--@ash/ashtop_by_username.sql sysdate+(1/1440*-60) sysdate
--
...
select distinct a.username, (select PCT from ashtop where session_state='WAITING' and username=a.username) Pct_of_This, 
(select "%User" from ashtop where session_state='WAITING' and username=a.username) WAITING_PCT, 
(select "%User" from ashtop where session_state='ON CPU' and username=a.username) CPU_PCT
...
FROM (
    SELECT /*+ LEADING(a) USE_HASH(u) */
	  ...
	  , ROUND(RATIO_TO_REPORT(COUNT(*)) OVER (Partition by username) * 100) "%User"
      , LPAD(ROUND(RATIO_TO_REPORT(COUNT(*)) OVER () * 100)||'%',5,' ')||' |' "%This"
      , session_state,session_type,username
      , TO_CHAR(MIN(sample_time), 'YYYY-MM-DD HH24:MI:SS') first_seen
      , TO_CHAR(MAX(sample_time), 'YYYY-MM-DD HH24:MI:SS') last_seen
      , COUNT(DISTINCT sql_exec_start||':'||sql_exec_id) dist_sqlexec_seen
    FROM
