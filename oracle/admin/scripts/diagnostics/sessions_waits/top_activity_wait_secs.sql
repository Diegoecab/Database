...
COLUMN SAMPLE_TIME for a25 HEADING 'Sample|Time'
...
SELECT SAMPLE_TIME,
	   ROUND(OTHER ) AS OTHER,
       ROUND(CLUST ) AS CLUST,
       ROUND(QUEUEING ) AS QUEUEING,
       ROUND(NETWORK ) AS NETWORK,
       ROUND(ADMINISTRATIVE ) AS ADMINISTRATIVE,
       ROUND(CONFIGURATION ) AS CONFIGURATION,
       ROUND(COMMIT ) AS COMMIT,
       ROUND(APPLICATION ) AS APPLICATION,
       ROUND(CONCURRENCY ) AS CONCURRENCY,
       ROUND(SIO ) AS SYSTEM_IO,
       ROUND(UIO ) AS USER_IO,
       ROUND(SCHEDULER ) AS SCHEDULER,
       ROUND(CPU ) AS CPU,
       ROUND(BCPU ) AS BACKGROUND_CPU,
		(SELECT count(*) FROM gv$active_session_history a where  
			SAMPLE_TIME=ASH.SAMPLE_TIME and session_type = 'FOREGROUND'
		) as AAS,
...
where name='cpu_count') as CPU_CNT
  FROM (SELECT SAMPLE_TIME,
         ...
         WHERE (SAMPLE_TIME > SYSDATE - INTERVAL '&1' SECOND)
           ) ASH PIVOT(COUNT(*) 
  FOR WAIT_CLASS IN(
