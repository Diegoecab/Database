-- ------------------------------------------------------------------------------
-- File       : sysmetric_history_db_wait_time_ratio.sql
-- Purpose    : Oracle diagnostic query/report helper: sysmetric history db wait time ratio.
-- Category   : diagnostics/sessions_waits
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : @sysmetric_history_db_wait_time_ratio.sql
...
-- ------------------------------------------------------------------------------
--
--sysmetric_history_db_wait_time_ratio minutes
set pages 1000
set verify off
...
select to_char(begin_time,'dd-mm-yyyy hh24:mi') btime,
 ...
 and METRIC_NAME in ('Database Wait Time Ratio')
 ORDER BY 1
/
...
select avg(value) "Average Wait time %", &_Average avg_hist, &_standard_deviation stddev, round(avg(value)/&_standard_deviation,1) timesplusstddev
 from gv$sysmetric_history
 where 
 begin_time > SYSDATE - INTERVAL '&1' MINUTE
 and METRIC_NAME in ('Database Wait Time Ratio')
