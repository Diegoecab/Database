-- ------------------------------------------------------------------------------
-- File       : TrendSGAstats.sql
-- Purpose    : Oracle SQL performance and tuning helper: TrendSGAstats.
-- Category   : performance/sql_tuning
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : @TrendSGAstats.sql
...
from (select '"'||name||'"' n, begin_interval_time, round(bytes/1024/1024) bytes from dba_hist_sgastat a, dba_hist_snapshot b 
where pool='shared pool' and a.snap_id=b.snap_id
and to_char(begin_interval_time,'hh24:mi') between '01:00' and '24:00'
and to_char(begin_interval_time,'dd-mon')  = '17-jan')--to_char(sysdate-1, 'dd-mon'))
group by n;
--spool off
