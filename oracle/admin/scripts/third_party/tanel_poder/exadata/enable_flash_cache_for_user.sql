-- ------------------------------------------------------------------------------
-- File       : enable_flash_cache_for_user.sql
-- Purpose    : oracle/admin third_party helper: enable flash cache for user.
-- Engine     : oracle/admin
-- Category   : third_party
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : Run with the target database client: enable_flash_cache_for_user.sql
-- Parameters : Review script body before running.
-- Risk       : CHANGES
-- Output     : Client, shell or script-defined output.
-- Notes      : Validate in a non-production environment before operational use.
-- Source     : internal
-- Change Log :
-- 2026-05-11 : Diego Cabrera - Header normalization.
-- ------------------------------------------------------------------------------
--
-- Copyright 2018 Tanel Poder. All rights reserved. More info at http://tanelpoder.com
-- Licensed under the Apache License, Version 2.0. See LICENSE.txt for terms & conditions.

DECLARE
    cmd VARCHAR2(1000);
BEGIN
    FOR i IN (SELECT owner,table_name FROM dba_tables WHERE owner = '&1') LOOP
        cmd := 'ALTER TABLE '||i.owner||'.'||i.table_name||' STORAGE (CELL_FLASH_CACHE KEEP)';
        DBMS_OUTPUT.PUT_LINE(cmd);
        EXECUTE IMMEDIATE cmd;
    END LOOP;

    FOR i IN (SELECT owner,index_name FROM dba_indexes WHERE owner = '&1') LOOP
        cmd := 'ALTER INDEX '||i.owner||'.'||i.index_name||' STORAGE (CELL_FLASH_CACHE KEEP)';
        DBMS_OUTPUT.PUT_LINE(cmd);
        EXECUTE IMMEDIATE cmd;
    END LOOP;
END;
/

