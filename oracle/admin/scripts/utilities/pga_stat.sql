-- ------------------------------------------------------------------------------
-- File       : pga_stat.sql
-- Purpose    : Oracle administration helper: pga stat.
-- Category   : utilities
...
select vses.inst_id, vses.username||':'||vsst.sid||','||vses.serial#||','||vses.inst_id username, vstt.name, max(vsst.value) value 
...
group by  vses.inst_id,vses.username, vsst.sid, vses.serial#, vstt.name order by value; 


 alter system kill session '49,50446,@1' immediate;


Prompt Historic usage
