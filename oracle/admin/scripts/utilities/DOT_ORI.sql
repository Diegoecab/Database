-- ------------------------------------------------------------------------------
-- File       : DOT_ORI.sql
-- Purpose    : Oracle administration helper: DOT ORI.
-- Category   : utilities
  ...
  SELECT s.empnro,
     ...
     FROM garbarrhh.ina_reportes r,
          (SELECT   rep_id,
                    ...
                    MAX (motivo_dot_id) motivo_dot_id,
                    SUM (rango1) rango1,
                    SUM (rango2) rango2,
                    SUM (rango3) rango3,
                    SUM (rango4) rango4,
    ...
    WHERE rd.motivo_dot_id = m.motivo_dot_id(+) and r.rep_id = rd.rep_id AND p.persona_id = rd.persona_id AND r.suc_id = s.suc_id AND s.empnro = e.empnro AND f.ID(+) = rd.funcion_id AND r.rep_id = re.rep_id ;
