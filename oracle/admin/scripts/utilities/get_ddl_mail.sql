-- ------------------------------------------------------------------------------
-- File       : get_ddl_mail.sql
-- Purpose    : Oracle administration helper: get ddl mail.
-- Category   : utilities
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : @get_ddl_mail.sql
...
exception
when others then
	dbms_output.put_line('Error looking for obj dependents or no dependents');
end;
...
/* send mail! */
	...
	begin
		...
		FOR i IN 0 .. TRUNC((DBMS_LOB.getlength(v_def||v_obj_grants) - 1 )/l_step) 
		LOOP
			UTL_SMTP.write_data(l_mail_conn, DBMS_LOB.substr(v_def||v_obj_grants, l_step, i * l_step + 1));
		END LOOP;
