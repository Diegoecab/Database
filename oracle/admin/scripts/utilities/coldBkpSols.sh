...
# ------------------------------------------------------------------------------
# File       : coldBkpSols.sh
# Purpose    : Oracle administration helper: coldBkpSols.
# Category   : utilities
# Author     : Diego Cabrera
# Created    : Unknown
# Version    : 1.0
# Usage      : ./coldBkpSols.sh
...
PTHTMP=/opt/oracledb/backup/prod01 ; export PTHTMP
...
echo    "       dbms_output.put_line ('cp '||loc_record.NAME||' /opt/oracledb/backup/prod01/.');  " >>$PTHTMP/cptemp.sql
...
echo    "       host gzip -f /opt/oracledb/backup/prod01/* " >>$PTHTMP/cptemp.sql
echo    "       host date       " >>$PTHTMP/cptemp.sql
echo    "       Prompt Fin de Backup " >>$PTHTMP/cptemp.sql
echo    "       quit    " >>$PTHTMP/cptemp.sql

#sqlplus sys/Solsprod01#@prod01 as sysdba @$PTHTMP/cptemp.sql >> $PTHTMP"/log_backup_full_"$FECHA"_"$NOMBRE_BASE".log"
