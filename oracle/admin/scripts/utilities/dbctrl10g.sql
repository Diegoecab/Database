-- ------------------------------------------------------------------------------
-- File       : dbctrl10g.sql
-- Purpose    : Oracle administration helper: dbctrl10g.
-- Category   : utilities
...
select name, substr(value,1,200) value from v$parameter where name like '%log_archive%';
...
select job, log_user, what, last_date,last_sec,next_date,next_sec,broken from dba_jobs order by job;

/********************************
los sigs. dos scripts tienen error ya que no toman en cuenta los objetos en null para el mínimo
ojo controlar y modificar
***********************************/
...
col name format a40 
col value format a200
Select name, substr(value,1,200) value from v$parameter;

prompt
prompt ---------------------------------------------------------------
prompt

-- controlar si está corriendo el job de estadísticas en 10g
-- poner que muestre cuando se corriò por ùltima vez el awr de las estadìsticas

select round(sum(bytes)/1024/1024) Datos from dba_segments;

select round(sum(bytes)/1024/1024) Base from dba_data_files; -- + dba_temp_files;

--////////////////////////////////////////////--

-- verificar el tipo de trace del listener

-- controlar el tamaño de los archivos de log y trace del listener que no sean mayor a 10 M
-- comprimirlos, ojo que si llega a dos G revienta la base

-- depuración de archives y espacio físico restante
