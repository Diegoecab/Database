...
--------------------------------------------------------------------------------
-- 
-- File name:  dba_hist_sysmetric_summary_io_js.sql v1.0
-- Purpose:    
-- Usage:       
--     @dba_hist_sysmetric_summary_io_js <lastNminutes>
--
-- Example:
--     Last hour
--     @dba_hist_sysmetric_summary_io_js 60
*/
...
dbms_output.put_line('var config3 = {'); --Begin var config
...
dbms_output.put_line	('{'); --Begin dataset 1 Physical_Read_Total_MBps
dbms_output.put_line('			label: ''Physical Read Total MBps'',');
...
dbms_output.put_line	('y: '||r.Physical_Read_Total_MBps);
...
FOR r IN cursorValue LOOP
if cursorValue%ROWCOUNT = 1 then 
dbms_output.put_line	('{'); --Begin dataset 2 Physical_Write_Total_MBps
dbms_output.put_line('			label: ''Physical Write Total MBps'',');
...
dbms_output.put_line	('x: newDate('''||r.sample_time||'''),');
dbms_output.put_line	('y: '||r.Physical_Write_Total_MBps);
dbms_output.put_line	('}');--End list of data
END LOOP;
...
FOR r IN cursorValue LOOP
if cursorValue%ROWCOUNT = 1 then 
dbms_output.put_line	('{'); --Begin dataset 3 Redo_MBytes_per_sec
dbms_output.put_line('			label: ''Redo MBytes per sec'',');
...
dbms_output.put_line	('x: newDate('''||r.sample_time||'''),');
dbms_output.put_line	('y: '||r.Redo_MBytes_per_sec);
dbms_output.put_line	('}');--End list of data
END LOOP;
...
FOR r IN cursorValue LOOP
if cursorValue%ROWCOUNT = 1 then 
dbms_output.put_line	('{'); --Begin dataset 14 Network_Mbytes_per_sec %
dbms_output.put_line('			label: ''Network Mbytes per sec'',');
...
dbms_output.put_line	('x: newDate('''||r.sample_time||'''),');
dbms_output.put_line	('y: '||r.Network_Mbytes_per_sec);
dbms_output.put_line	('}');--End list of data
END LOOP;
