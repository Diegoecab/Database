-- ------------------------------------------------------------------------------
-- File       : refrescos1.sql
-- Purpose    : Oracle administration helper: refrescos1.
-- Category   : utilities
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : @refrescos1.sql
-- Parameters : Review ACCEPT variables and substitution variables before running.
-- Requires   : SQL*Plus or SQLcl and privileges required by referenced dictionary views.
-- Oracle Ver.: Review compatibility before production use.
-- Risk       : READ ONLY
-- Output     : SQL*Plus/SQLcl console or spool output.
-- Notes      : Validate in a non-production session before operational use.
-- Source     : internal
-- Change Log : 
-- 2026-05-11 : Diego Cabrera - Header normalization.
-- ------------------------------------------------------------------------------
--
conectarse en el servidor de aplicaciones nuevo como sys

-- Despues esto

DECLARE
  CURSOR c_cursor IS SELECT c.scd_surrgt_pk, c.scd_ins_surrgt_fk, COUNT(a.cch_url) cant_work 
                       FROM discoverer5.ptm5_schedule c,          
                            discoverer5.ptm5_cache a
                      WHERE a.cch_ins_surrgt_fk (+) = c.scd_ins_surrgt_fk
                        AND c.scd_next_update < SYSDATE + 2 
--                        AND TRUNC(scd_next_update) <= TRUNC(SYSDATE) -- solo los pendientes de hoy
 				   GROUP BY c.scd_surrgt_pk, c.scd_ins_surrgt_fk;          
  v_minuto NUMBER := 1/24/60;
  v_number NUMBER;
--v_fecha DATE := SYSDATE; 
  v_fecha DATE := TO_DATE( '2008-06-20:13:55:00','YYYY/MM/DD:HH24:MI:SS'); -- poner fecha y hora desde la cual se refrescara 
BEGIN  
  FOR c IN c_cursor LOOP
    v_number := v_minuto * c.cant_work / 3; -- estimamos 20 segundos por trabajo
    DBMS_OUTPUT.put_line(v_fecha);   
    
    UPDATE discoverer5.ptm5_schedule 
       SET scd_next_update = v_fecha
     WHERE scd_ins_surrgt_fk = c.scd_ins_surrgt_fk;  
     
    UPDATE discoverer5.ptm5_cache a  
       SET cch_next_update = v_fecha + 0.0002
     WHERE a.cch_ins_surrgt_fk = c.scd_ins_surrgt_fk;

    v_fecha := v_fecha + v_number;
  END LOOP;
END; 


