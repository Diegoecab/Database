-- ------------------------------------------------------------------------------
-- File       : dba_hist_system_event_io2.sql
-- Purpose    : Oracle administration helper: dba hist system event io2.
-- Category   : utilities
-- Author     : Diego Cabrera
-- Created    : Unknown
-- Version    : 1.0
-- Usage      : @dba_hist_system_event_io2.sql
...
-- ------------------------------------------------------------------------------
--
--dba_hist_system_event_io2

column 1 format 999999 heading inst_1
column 2 format 999999 heading inst_2
column 3 format 999999 heading inst_3
column 4 format 999999 heading inst_4
column 5 format 999999 heading inst_5
column 6 format 999999 heading inst_6
...
where  event_name  in 
( 'cell single block physical read', 'log file parallel write', 'log file sync','cell multiblock physical read',  'direct path write',  'direct path read', 'direct path read temp', 'direct path write temp','db file sequential read', 'db file scattered read')
and     snap_id > (select begin_snap_id from pivot0) and snap_id < (select end_snap_id from pivot0)
)
,pivot2 as
(
select instance_number,snap_id, end_time, event_name,
