-- ------------------------------------------------------------------------------
-- File       : imp_r.sql
-- Purpose    : Oracle administration helper: imp r.
-- Category   : utilities
...
select q.job_id,s.host, s.sid, s.schema,to_char(q.book_date,'DD-MON-YY') BOOK_DATE,q.run,
       q.imp_mode, 
       ...
       q.status, 
       q.tgt_host,q.tgt_sid,q.tgt_schema,
       q.submitted,
       Q.OBJ_DROPPED, 
       Q.OBJ_MISS,ROUND(q.prod_size/1024/1024/1024) PRO_GB,        ROUND(q.job_size/1024/1024/1024) JOB_GB,
       q.started,
       q.files,
       (select count(1) from dbadmin.imp_dumpfile where job_id= q.job_id and status='SENT') ||'/'||
       (select count(1) from dbadmin.imp_dumpfile where job_id= q.job_id) tot
